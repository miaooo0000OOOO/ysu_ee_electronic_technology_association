...
{
    if (vec->size + size - 1 == vec->capacity)
    {
        ItemType *data = (ItemType *)realloc(vec->data, vec->capacity * 2 * sizeof(ItemType));
        vec->data = data;
        vec->capacity *= 2;
    }
    if (size < vec->capacity / 2)
    {
        vec_mem_prepare(vec, size - vec->capacity / 2);
    }
}
...
2. 实现vec_find函数，`int vec_find(ItemType val)`，输入一个值val，返回这个值对应的索引，如果有多个值相同，返回索引的最小值，找不到返回-1
...
但是链表的各个元素在内存之间不连续，这会极大地降低缓存命中率
